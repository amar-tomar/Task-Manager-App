...
const CreateTask = () => {
  ...
  const validateTaskData = () => {
    ...
    if (taskData.assignedTo.length === 0) return "Assign the task to at least one member.";
    if (taskData.todoChecklist.length === 0) return "Add at least one todo task.";
    return "";
  };
  ...
  const getTaskDetailsById = async () => {
    try {
      const response = await axiosInstance.get(apiPaths.tasks.getTaskById(taskId));
